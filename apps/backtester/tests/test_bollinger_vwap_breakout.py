"""Unit tests for BollingerVwapBreakoutStrategy -- directly drives Strategy.refresh()
against a controlled single-symbol Market with hand-picked VWAP/Bollinger-band/squeeze
series, so entry direction, SL/TP levels, and exits are asserted exactly."""
...
from backtester.strategies import BollingerVwapBreakoutStrategy, CloseReason, StrategyDirection
...
def _market(rows: list[dict]) -> Market:
    """Each row: close, high, low, vwap, bb_lower, bb_upper, is_squeeze, and
    ...
    market.add_indicator(
        symbol=SYMBOL, interval=INTERVAL, unit_of_time=UNIT_OF_TIME,
        indicator_name=BB_LOWER_KEY, df=pd.Series([r["bb_lower"] for r in rows], index=index),
    )
    market.add_indicator(
        symbol=SYMBOL, interval=INTERVAL, unit_of_time=UNIT_OF_TIME,
        indicator_name=BB_UPPER_KEY, df=pd.Series([r["bb_upper"] for r in rows], index=index),
    )
    market.add_indicator(
        symbol=SYMBOL, interval=INTERVAL, unit_of_time=UNIT_OF_TIME,
...
def _strategy(market: Market, **kwargs) -> BollingerVwapBreakoutStrategy:
    return BollingerVwapBreakoutStrategy(
        key="test", market=market, symbol=SYMBOL,
        vwap_indicator_key=VWAP_KEY, bb_lower_key=BB_LOWER_KEY, bb_upper_key=BB_UPPER_KEY,
        squeeze_indicator_key=SQUEEZE_KEY, **kwargs,
    )


def _run(market: Market, strategy: BollingerVwapBreakoutStrategy, n: int) -> None:
    for i in range(n):
        market.set_current_market_from_num_index(i)
        strategy.refresh()


class TestBollingerVwapBreakoutStrategy:
    def test_enters_long_when_squeezed_and_above_vwap(self):
        # price=105 > vwap=100, squeeze True -> long, sl=lower_band=95, risk=10,
        # tp = 105 + 2*10 = 125.
        market = _market([
            {"close": 105, "high": 105, "low": 105, "vwap": 100, "bb_lower": 95, "bb_upper": 115, "is_squeeze": True},
        ])
    ...
    def test_enters_short_when_squeezed_and_below_vwap(self):
        ...
        # tp = 95 - 2*10 = 75.
        market = _market([
            {"close": 95, "high": 95, "low": 95, "vwap": 100, "bb_lower": 85, "bb_upper": 105, "is_squeeze": True},
        ])
        strategy = _strategy(market)
        _run(market, strategy, 1)
    ...
    def test_no_entry_without_a_squeeze(self):
        market = _market([
            {"close": 105, "high": 105, "low": 105, "vwap": 100, "bb_lower": 95, "bb_upper": 115, "is_squeeze": False},
        ])
        strategy = _strategy(market)
        _run(market, strategy, 1)
    ...
    def test_exits_on_stop_loss(self):
        ...
        market = _market([
            {"close": 105, "high": 105, "low": 105, "vwap": 100, "bb_lower": 95, "bb_upper": 115, "is_squeeze": True},
            {"close": 96, "high": 106, "low": 94, "vwap": 100, "bb_lower": 95, "bb_upper": 115, "is_squeeze": False},
        ])
        strategy = _strategy(market)
    ...
    def test_exits_on_take_profit(self):
        ...
        market = _market([
            {"close": 105, "high": 105, "low": 105, "vwap": 100, "bb_lower": 95, "bb_upper": 115, "is_squeeze": True},
            {"close": 120, "high": 126, "low": 104, "vwap": 100, "bb_lower": 95, "bb_upper": 115, "is_squeeze": False},
        ])
        strategy = _strategy(market)
        _run(market, strategy, 2)
    ...
    def test_skips_entry_when_risk_is_not_positive(self):
        # price=105 > vwap -> long side, but bb_lower=110 is *above* price -- a
        # degenerate/invalid band placement with no positive risk distance to size
        # the trade or its take-profit off.
        market = _market([
            {"close": 105, "high": 105, "low": 105, "vwap": 100, "bb_lower": 110, "bb_upper": 115, "is_squeeze": True},
        ])
        strategy = _strategy(market)
        _run(market, strategy, 1)
    ...
    def test_risk_reward_ratio_is_configurable(self):
        market = _market([
            {"close": 105, "high": 105, "low": 105, "vwap": 100, "bb_lower": 100, "bb_upper": 115, "is_squeeze": True},
        ])
    ...
    def test_trend_filter_blocks_long_when_price_is_below_trend(self):
        ...
        market = _market([
            {"close": 105, "high": 105, "low": 105, "vwap": 100, "bb_lower": 95, "bb_upper": 115,
    ...
    def test_trend_filter_blocks_short_when_price_is_above_trend(self):
        market = _market([
            {"close": 95, "high": 95, "low": 95, "vwap": 100, "bb_lower": 85, "bb_upper": 105,
