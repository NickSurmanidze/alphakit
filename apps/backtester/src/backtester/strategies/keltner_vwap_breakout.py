"""Keltner squeeze breakout strategy: enters in the direction VWAP indicates while
Keltner Channels are unusually narrow, with a band-derived stop-loss and a fixed
risk:reward take-profit. Same shape as BollingerVwapBreakoutStrategy, with
ATR-based Keltner bands in place of std-dev-based Bollinger Bands -- ATR reacts to
realized range rather than the variance of closes, so Keltner bands tend to hold
up better than std-dev bands specifically in trending markets."""
...
class KeltnerVwapBreakoutStrategy(Strategy):
    """Enters a trade the moment Keltner Channels are in a "squeeze" (a
    precomputed boolean indicator -- this strategy doesn't compute the
    squeeze/percentile logic itself, same convention as
    BollingerVwapBreakoutStrategy consuming precomputed indicators rather than
    owning the statistics). Direction is decided by price's position relative to
    session VWAP at that moment: above VWAP -> long, below -> short -- betting the
    squeeze resolves in the direction the session's volume-weighted "fair value"
    already leans.

    Stop-loss sits at the *opposite* band (lower band for a long, upper band for a
    short) -- during a squeeze the bands are close together, so this is naturally
    a tight, volatility-scaled stop. Take-profit is `risk_reward_ratio` times that
    same risk distance (default 2.0, i.e. risking 1 to make 2).
    ...
    above it, a short only if also below it -- same convention (and same
    parameter name) as BollingerVwapBreakoutStrategy's own trend filter. `None`
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.kc_lower_key: str = kc_lower_key
        self.kc_upper_key: str = kc_upper_key
    ...
    def refresh(self) -> None:
        ...
        lower_band = indicators[self.kc_lower_key]
        upper_band = indicators[self.kc_upper_key]
